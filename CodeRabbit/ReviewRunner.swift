...
final class ReviewRunner: ObservableObject {
    ...
    private let staleRunRecoveryGraceSeconds: TimeInterval = 8
    ...
    func restoreRateLimitCooldown(from historyItems: [ReviewHistoryItem], referenceDate: Date = Date()) {
        guard currentRunID == nil else { return }

        var latestCooldownEnd: Date?
        for item in historyItems {
            guard let errorInfo = ReviewParser.parseRunErrorInfo(from: item.rawOutput),
                  errorInfo.isRateLimit,
                  let retryAfterSeconds = errorInfo.retryAfterSeconds
            else {
                continue
            }

            let occurredAt = errorInfo.occurredAt ?? item.createdAt
            let cooldownEnd = occurredAt.addingTimeInterval(retryAfterSeconds)
            guard cooldownEnd > referenceDate else { continue }

            if let existing = latestCooldownEnd {
                if cooldownEnd > existing {
                    latestCooldownEnd = cooldownEnd
                }
            } else {
                latestCooldownEnd = cooldownEnd
            }
        ...
        nextAllowedRunAt = latestCooldownEnd
    }
    ...
    func recoverInterruptedRunIfNeeded(referenceDate: Date = Date()) {
        guard case .running = status else { return }
        if process?.isRunning == true { return }
        if let startedAt = currentRunStartedAt,
           referenceDate.timeIntervalSince(startedAt) < staleRunRecoveryGraceSeconds {
            return
        }
        ...
        return await withCheckedContinuation { (continuation: CheckedContinuation<GitChangeSummary?, Never>) in
            DispatchQueue.global(qos: .userInitiated).async {
                ...
                if let nonEmptyBaseRef {
                    let mergeBaseOutput = self.runGitCommand(
                        arguments: ["merge-base", nonEmptyBaseRef, "HEAD"],
                        folderPath: trimmedPath
                    )
                    guard mergeBaseOutput.exitCode == 0 else {
                        continuation.resume(returning: nil)
                        return
                    }
                    let mergeBase = mergeBaseOutput.output.trimmingCharacters(in: .whitespacesAndNewlines)
                    guard !mergeBase.isEmpty else {
                        continuation.resume(returning: nil)
                        return
                    }
                    // Diff merge-base(base, HEAD) against the working tree to include:
                    // - committed branch changes since divergence from base
